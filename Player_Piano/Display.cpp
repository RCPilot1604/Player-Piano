    ...
    case 3: //MinPWM || MaxPWM
      if (flashing) { //if we are done editing the settings
        if (currentSubstate == 1) {
          if (placeholder1 != mySettings.getMinDPWM()) { //we only write into our NVS if the data has changed
            ...
            if(mynvs.setInt("MIN_PWM",placeholder1)){
              #ifdef MEMORY_DEBUG
     ...
     case 4: //Normal < || Bounce <
      ...
      if(currentSubstate == 1){
        ...
        int pwm = notes[CURRENT_NOTE-mySettings.MIN_NOTE_ID].calculatePWM(velocity);
...
void Display::setupNVS(){
  ...
  int savedMaxPWM = mynvs.getInt("MAX_PWM");
  if(savedMaxPWM == 0){
    ...
    mySettings.setMaxDPWM(mySettings.MAX_PWM); //we set the max pwm parameter to the default value
  }else{
    mySettings.setMaxDPWM(savedMaxPWM); //we set the max pwm parameter to the value retrieved from the NVS
    #ifdef MEMORY_DEBUG
    Serial.print("RETRIEVED saved value of MaxPWM "); Serial.println(mySettings.getMaxDPWM());
    #endif
  }
  int savedMinPWM = mynvs.getInt("MIN_PWM");
  if(mySettings.MIN_PWM != 0 && savedMinPWM == 0){ //the default mySettings.MIN_PWM is not zero but the nvs has saved it as zero
    ...
    mySettings.setMinDPWM(mySettings.MIN_PWM);
  }else{
    mySettings.setMinDPWM(savedMinPWM); 
    #ifdef MEMORY_DEBUG
